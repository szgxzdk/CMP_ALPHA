...
class sim_router_template {
	...
	public:
		...
		int isBufferFull() {return counter_ <= 0;}
		void counter_inc() {counter_++;}
		void counter_dec() {counter_--;}
		...
		double power_report(FILE *fd, int telem) {return power_module_.power_report(4*address_[0]+address_[1], fd, telem);}
