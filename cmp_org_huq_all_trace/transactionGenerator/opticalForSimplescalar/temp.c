...
counter_t time_schedule(Packet packet, counter_t current_time, int busy_flag, int *tran_type)
{
	...
	if(packet.msgno == 151652 || packet.msgno == 151667)
		printf("here\n");
	...
	if(*tran_type != 1)
	{
		if((schedule_time >= transmitter_backup_schedule[src]) && (!is_transmitter_backup_empty(src)))
		{
...
#endif
	{
		schedule_time = time_schedule(packet, current_time, busy_flag, &tran_type);
		...
		input_buffer_add(packet, schedule_time + laser_setup_time + flit_tranfer_time);
...
#ifdef PSEUDO_SCHEDULE_REORDER
void transfer_queue(Packet pck)
{
	Flit *flit, *new_flit;
	int i = 0, src = pck.src;
	...
	if(last_packet_des[src] != pck.des && delay_counter[src] + 2 > pck.transfer_time)
		steering_delay_adjust(pck.msgno);	
	last_packet_des[src] = pck.des;	 
	delay_counter[src] = pck.transfer_time;
	laser_power_on_cycle[src] = delay_counter[src];
}

void steering_delay_adjust(int msgno)
{
	Packet packet;
	Msg *current_msg;
	...
	{
		if (current_msg->msg_type != META_MSG)
		...
		if(packet.msgno == msgno)
			current_msg->when = current_msg->when + laser_setup_time;

		steering_delay += laser_setup_time;
		current_msg = current_msg->next;
	}
	return 0;
}
