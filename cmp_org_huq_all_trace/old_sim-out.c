		...
		int threadid)
{
	...
	{
		oc_or_cc_access = 1;
		byte_t *l0cache_line = calloc (bsize, sizeof (byte_t));

		cacheBcopy (Read, blk, 0, l0cache_line, bsize);

#ifdef  LDTH_USEONLY_L2
		/* access next level of data cache hierarchy */
		lat = cache_access (cache_dl2, cmd, (baddr & ~(bsize - 1)), l0cache_line, bsize,
				/* now */ now, /* pudata */ NULL, /* repl addr */ NULL, threadid);
		l1HitFlag = 0;
#else
		/* access next level of data cache hierarchy */
		lat = cache_access (cache_dl1[threadid], cmd, (baddr & ~(bsize - 1)), l0cache_line, bsize,
				/* now */ now, /* pudata */ NULL, /* repl addr */ NULL, threadid);
#endif
		cacheBcopy (Write, blk, 0, l0cache_line, bsize);
		free (l0cache_line);

		oc_or_cc_access = 2;

#ifdef  HIGHER_LDTH_L1LAT
		lat += LDTH_L1LAT;
#endif
		if (cmd == Read)
			return lat;
	      ...
	      )
{
	...
	//if (cmd == Read)
	{
		...
		src = (baddr >> cache_dl2->set_shift) % MAXTHREADS;
		x = (src /MESH_SIZE);
		y = (src %MESH_SIZE);	
...
#if 0				/* not yet supported here */
	/*lechen, 2Bc-gskew predictor2 in EV8 */
	opt_reg_int_list (odb, "-bpred2:2bcgskew", "2Bc-gskew predictor2 config (<bim_size> <size> <bim_hist_size> <g0_hist_size> <g1_hist_size> <meta_hist_size>)", twobcgskew_config2, twobcgskew_nelt2, &twobcgskew_nelt2,
			/* default */ twobcgskew_config2,
			/* print */ TRUE, /* format */ NULL, /* !accrue */ FALSE);
#endif
...
sim_reg_stats (struct stat_sdb_t *sdb)	/* stats database */
{
	...
	{
		if (cache_dl0[i])
			cache_reg_stats (cache_dl0[i], sdb, i);
	}
#endif
...
int buffer_full_check(struct DIRECTORY_EVENT *event)
{
	...
	if((event->des1*MESH_SIZE+event->des2 != event->src1*MESH_SIZE+event->src2) && (event->input_buffer_full) && (event->operation == MISS_WRITE || event->operation == MISS_READ || event->operation == WRITE_UPDATE || event->operation == MISS_IL1))
	{
		...
		else if (!mystricmp (network_type, "MESH"))
		{
			...
			if(popnetBufferSpace(event->src1, event->src2, options) == -1)
				buffer_full = 1;
		...
		while (current->RUU_num > 0 && committed < ruu_commit_width)
		{
			...
			if(rs->writewait != 1)
			{
...
#if 0
				if((MD_OP_FLAGS (rs->op) & F_CTRL) && (current->id == 0))
				{
					fprintf(fp_trace, "[BR INST] PC: 0x%llx Insn Num: %lld cycle: %lld\n", rs->PC, rs->sim_num_insn, sim_cycle);
					fflush(fp_trace);
				}
#endif
#ifdef	EDA
				if (MD_OP_FLAGS (rs->op) & F_CTRL)
				{
					if (current->masterid == 0)
					{
						commitPrediction (current->predQueue, rs->boqIndex);

						dynConfBOQSize (current->predQueue);

						if (rs->recover_inst && rs->lockStatus != TYPE_LOCK_ACQUIRE && rs->lockStatus != TYPE_INST_BARRIER)
							edaRecovery (rs);
					}

					if (current->masterid == 1 && rs->instType != TYPE_INST_BARRIER_END)
					{
						branchInst *branchInfo = NULL;

						pushBranchPred (current->predQueue, &branchInfo, rs->op);
						branchInfo->inst = rs->IR;
						branchInfo->op = rs->op;
						branchInfo->PC = rs->PC;
						branchInfo->NPC = rs->next_PC;
						branchInfo->time = sim_cycle;
						branchInfo->sim_num_insn = rs->sim_num_insn;
						branchInfo->instType = rs->instType;
					}
...
#if 0				//def     EDA
						if (current->masterid == 1 && rs->out1 > 0 && rs->out1 < 63 && rs->poison[2].dirty && !rs->ea_comp)
						{
							if (rs->out1 < 32)
								current->regs.regs_R[31 - rs->out1] = rs->poison[2].data;
							else
								current->regs.regs_F.q[rs->out1 - 32] = rs->poison[2].data;
						}
#endif
					}
				}
...
#ifdef	EDA
						/* Propogate bad results */
						if (rs->poison[2].dirty)
						{
							olink->rs->poison[olink->x.opnum].dirty = 1;
							olink->rs->poison[olink->x.opnum].data = rs->poison[2].data;
						}
						else
						{
							olink->rs->poison[olink->x.opnum].dirty = 0;
							olink->rs->poison[olink->x.opnum].data = 0;
						}
#endif
					...
					if (MD_OP_FUCLASS (rs->op) != NA)
					{
						...
						if (fu)
						{
							...
							if (rs->in_LSQ && ((MD_OP_FLAGS (rs->op) & (F_MEM | F_LOAD)) == (F_MEM | F_LOAD)))
							{
								...
								int matchnum;
								matchnum = MSHR_block_check(cache_dl1[threadid]->mshr, addr, cache_dl1[threadid]->set_shift);
								if(matchnum && !rs->isPrefetch && !mshr_pending_event[threadid] && rs->sqStoreForwardT == -1)
								{
									cache_dl1[threadid]->in_mshr ++;
									rs->issued = FALSE;
									rs->cache_ready = 1;
...
#ifdef	RUN_ORACLE
			/* Execute the oracle */
			if (current->lockStatus == 0 && current->masterid == 1 && (current->spec_mode == 0) && (thecontexts[current->oracleId]->spec_mode == 0))
			{
				if (thecontexts[current->oracleId]->lockStatus == TYPE_LOCK_ACQUIRE || thecontexts[current->oracleId]->lockStatus == TYPE_INST_BARRIER)
				{		/* Try to execute one more inst of oracle until it reaches the current PC */
					execOracleEmul (thecontexts[current->oracleId]);
					if (thecontexts[current->oracleId]->regs.regs_PC != current->regs.regs_PC)
						break;
					else if (thecontexts[current->oracleId]->lockStatus != 0)
						panic ("Lock status must have been reset for the lead thread");
				}
				else if (current->regs.regs_PC != thecontexts[current->oracleId]->regs.regs_NPC)
					panic ("Opportunistic and Oracle must be in lock step\n");
				else
					execOracleEmul (thecontexts[current->oracleId]);
			}
#endif
			int instType = TYPE_INST_INCLUDED;

			if (current->masterid == 1)
				instType = GetExecStatus (current->regs.regs_PC, NULL);

			if(current->masterid == 0 && (MD_OP_FLAGS(op) & F_LOAD) && (RA == 31))
			{
				instType = TYPE_INST_REMOVED;
				if (!current->spec_mode)
				{
					pure_num_insn++;
					sim_num_insn++;
...
#ifdef	EDA
				if (current->masterid == 1 && MD_OP_FLAGS (op) & F_LOAD)
				{
					int loadLocation = 0;

					int memWidth = GetMemAccWidth (op);
					md_addr_t memAddr = (GPR (RB) + SEXT (OFS)) & (~(memWidth - 1));
					byte_t *memData = calloc (memWidth, sizeof (byte_t));

					if (cacheBlockSearch (cache_dl0[current->jobThdId], Read, memAddr, memData, memWidth) == 0)
					{
						if (cacheBlockSearch (cache_dl1[current->helpThdId], Read, memAddr, memData, memWidth) == 0)
						{
							/* First locate this cache line in other L1 caches.
							 ** If not found there, then take it from L2. */
							int tmpId, found = 0;

							for (tmpId = 0; tmpId < numcontexts; tmpId++)
							{
								if (thecontexts[tmpId]->masterid != 0)
									continue;
								if (tmpId == current->helpThdId)
									continue;

								if (cacheBlockSearch (cache_dl1[tmpId], Read, memAddr, memData, memWidth) == 1)
								{
									found = 1;
									break;
								}
							...
							if (found == 0 && cacheBlockSearch (cache_dl2, Read, memAddr, memData, memWidth) == 0)
							{
								mem_access_drct (thecontexts[current->helpThdId]->mem, Read, memAddr, memData, memWidth, current->helpThdId);
								loadLocation = 4;
							}
							else
								loadLocation = 3;
					...
					mem_access_drct (current->mem, Write, memAddr, memData, memWidth, current->id);
					free (memData);
				}

				if (current->masterid == 0 || (current->masterid == 1 && instType != TYPE_INST_PREFETCH && op != PAL_CALLSYS))
				{		/* We donot emulate prefetch instructions */
#endif
					switch (op)
					{
...
#include "machine.def"
				...
				else
				{
					...
					if(MD_OP_FLAGS (op) &  F_MEM)
					{
						...
						in2 = 31 - RB;
						val_rb = current->regs.regs_R[31 - in2];
					}
				}
#endif
				if (!current->spec_mode)
...
#ifdef	EDA
				if (instType == TYPE_LOCK_ACQUIRE)
				{		/* We allow lead thread to skip lock acquire operation */
					current->regs.regs_NPC = current->regs.regs_PC + sizeof (md_inst_t);
					current->regs.regs_R[MD_REG_RA] = current->regs.regs_PC + sizeof (md_inst_t);
				}

				if (instType == TYPE_INST_BARRIER && (MD_OP_FLAGS(op) & F_CTRL))
				{		/* We just start after the barrier */
					//if (!current->spec_mode && GetExecStatus (current->regs.regs_PC - sizeof (md_inst_t), NULL) == TYPE_INST_BARRIER)
					//	panic ("Start of barrier error\n");

					for (current->regs.regs_NPC = current->regs.regs_PC + sizeof (md_inst_t); GetExecStatus (current->regs.regs_NPC, NULL) == TYPE_INST_BARRIER; current->regs.regs_NPC = current->regs.regs_NPC + sizeof (md_inst_t));

					md_inst_t inst_temp;
					enum md_opcode op_temp;

					MD_FETCH_INST (inst_temp, thecontexts[current->helpThdId]->mem, current->regs.regs_NPC - sizeof (md_inst_t));
					MD_SET_OPCODE (op_temp, inst_temp);

					if (op_temp == BSR)
						current->regs.regs_R[MD_REG_RA] = current->regs.regs_NPC;
				}
#endif
...
#ifdef	EDA
				if (!current->spec_mode && current->masterid == 0 && (current->fetch_data[current->fetch_head]->lockStatus == TYPE_LOCK_ACQUIRE || current->fetch_data[current->fetch_head]->lockStatus == TYPE_INST_BARRIER))
				{
					numSyncInst[current->jobThdId]++;
					pure_sync_insn++;
				}

				if (current->masterid == 0 && (current->fetch_data[current->fetch_head]->lockStatus == TYPE_LOCK_ACQUIRE || current->fetch_data[current->fetch_head]->lockStatus == TYPE_INST_BARRIER)
						&& (((MD_OP_FLAGS (op) & (F_CTRL | F_UNCOND | F_INDIRJMP)) == (F_CTRL | F_UNCOND | F_INDIRJMP)) || current->regs.regs_PC == current->lockExitPC))
				{
					current->pred_PC = current->regs.regs_NPC;
				}

				if (!current->spec_mode && current->masterid == 0)
				{
					if (current->fetch_data[current->fetch_head]->lockStatus != TYPE_LOCK_ACQUIRE && current->fetch_data[current->fetch_head]->lockStatus != TYPE_INST_BARRIER
							&& GetExecStatus (current->regs.regs_PC, NULL) == TYPE_INST_INCLUDED)
					{
						nonSktInsn[current->jobThdId]++;
						pure_skt_insn++;
					}
				}

#ifdef	RUN_ORACLE
				if (current->masterid == 1 && current->spec_mode == 0 && thecontexts[current->oracleId]->spec_mode == 0 && thecontexts[current->oracleId]->lockStatus == 0
						&& (current->regs.regs_NPC != thecontexts[current->oracleId]->regs.regs_NPC))
				{
					thecontexts[current->oracleId]->spec_mode = 1;
				}
#endif
#endif
				...
				if (op != MD_NOP_OP)
				{
					...
					rs->prevReg = tempReg;
#ifdef	EDA
					rs->boqIndex = current->fetch_data[current->fetch_head]->boqIndex;

					rs->instType = instType;
					rs->lockStatus = current->fetch_data[current->fetch_head]->lockStatus;

					rs->poison[0].dirty = rs->poison[0].data = 0;
					rs->poison[1] = rs->poison[2] = rs->poison[0];
					rs->changeSpecMode = 0;
					if (current->masterid == 0)
					{
						rs->isWrongPath = 0;
					...
					else if (current->masterid == 1)
					{
						rs->isWrongPath = thecontexts[current->oracleId]->spec_mode;
						rs->altPC = thecontexts[current->oracleId]->regs.regs_NPC;
					}
					else
						panic ("No oracle instruction can go into timing simulation.");

					rs->o_val_ra = thecontexts[current->oracleId]->o_val_ra;
					rs->o_val_rb = thecontexts[current->oracleId]->o_val_rb;
					rs->o_val_rc = thecontexts[current->oracleId]->o_val_rc;


					//if(current->masterid == 1 && !current->spec_mode && !thecontexts[current->oracleId]->spec_mode && (rs->o_val_ra != val_ra || rs->o_val_rb != val_rb || rs->o_val_rc != val_rc))
					//    fprintf(stderr, "Lead thread error\n");
#endif
					...
					if ((MD_OP_FLAGS (op) & F_MEM) && second_run)
					{
...
#ifdef	EDA
						lsq->boqIndex = current->fetch_data[current->fetch_head]->boqIndex;
						lsq->lockStatus = current->fetch_data[current->fetch_head]->lockStatus;
						lsq->poison[0].dirty = lsq->poison[0].data = 0;
						lsq->poison[1] = lsq->poison[2] = lsq->poison[0];
						lsq->changeSpecMode = 0;
						if (current->masterid == 0)
						{
							lsq->isWrongPath = 0;
							lsq->altPC = current->regs.regs_NPC;
						}
						else if (current->masterid == 1)
						{
							lsq->isWrongPath = thecontexts[current->oracleId]->spec_mode;
							lsq->altPC = thecontexts[current->oracleId]->regs.regs_NPC;
						}
						else
							panic ("No oracle instruction can go into timing simulation.");

						lsq->o_val_ra = thecontexts[current->oracleId]->o_val_ra;
						lsq->o_val_rb = thecontexts[current->oracleId]->o_val_rb;
						lsq->o_val_rc = thecontexts[current->oracleId]->o_val_rc;
						lsq->instType = instType;
#endif
						lsq->isPrefetch = 0;
						lsq->isL1Miss = 0;
						...
						if ((MD_OP_FLAGS (op) & (F_MEM | F_LOAD)) == (F_MEM | F_LOAD))
						{
							...
							if (current->masterid != 0)
							{
								if (instType == TYPE_INST_PREFETCH)
									lsq->isPrefetch = 1;
								else
									current->m_shLQNum++;
							}
						...
						{
#ifdef	EDA
							if (current->masterid == 1 && instType == TYPE_INST_PREFETCH)
								lsq->isPrefetch = 1;
							else
#endif
								current->m_shSQNum++;
...
#ifdef	EDA
			current->fetch_data[current->fetch_tail]->boqIndex = current->boqIndex;
			current->fetch_data[current->fetch_tail]->lockStatus = current->lockStatus;
#endif
