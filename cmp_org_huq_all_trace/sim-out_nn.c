...
#include "machine.def"
				...
				if (op != MD_NOP_OP)
				{
					...
					rs->isSyncInst = (collect_lock_stats[threadid] && isLockInst(current->regs.regs_PC));
					...
					if ((MD_OP_FLAGS (op) & F_MEM) && second_run)
					{
						...
						lsq->isSyncInst = (collect_lock_stats[threadid] && isLockInst(current->regs.regs_PC));
...
#define PRINT_STATISTICS 0
		...
		{
#ifdef SYNC_IDEAL
			if(allForked && (barrier_waiting[i]))
				no_ideal = 0;
#endif
			int lat = 100;
			if (!allForked)
