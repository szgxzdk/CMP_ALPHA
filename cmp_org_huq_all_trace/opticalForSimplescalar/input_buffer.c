...
int packet_type_check(Packet packet, counter_t sim_cycle)
{
	...
	if (event!=NULL)
	{
		while (event)
		{
			if(event->src1*(mesh_size+2)+event->src2 == packet.src && event->des1*(mesh_size+2)+event->des2 == packet.des && event->startCycle == packet.start_time && packet.msgno == event->popnetMsgNo)
			{
