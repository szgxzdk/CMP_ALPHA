...
void directRunSim(counter_t sim_cycle)
{
	...
	MetaConfPacketListClean(sim_cycle);
		...
		if(meta_conf_event)
		{
			...
			{
				case 1: req_conf[0] ++; break;
				case 2: req_conf[1] ++; break;
				case 3: req_conf[2] ++; break;
				default:  req_conf[3] ++; break;
			}
			meta_conf_event = 0;
		}
#endif
...
int finishMsg(long src, long des, counter_t start_time, counter_t popnetmsgno, int packet_type, counter_t transfer_time, int meta_delay, counter_t req_time, int operation, int out_buffer_dec)
{
	...
	if(operation >= 100)
	{
		if(LockMsgComplete(src1, src2, des1, des2, popnetmsgno))
			complete_flag = 1;
	}
	else 
#endif
	{
		if(MsgComplete(src1, src2, des1, des2, start_time, popnetmsgno, transfer_time, meta_delay, req_time))
			complete_flag = 1;
	}
	if(complete_flag)
	{
