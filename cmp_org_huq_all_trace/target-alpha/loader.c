 ...
 *
 * $Id: loader.c,v 1.1.1.1 2011/03/29 01:04:35 huq Exp $
 *
 * $Log: loader.c,v $
 * Revision 1.1.1.1  2011/03/29 01:04:35  huq
 * setup project CMP
 *
 * Revision 1.1.1.1  2010/02/18 21:22:48  xue
 * CMP_NETWORK
 *
...
#ifdef SMT_SS
/*    SMT version of ld_reg_stats  and  ld_load_prog    functions  */
             ...
             int threadid)               /*  context id */
{
  ...
  if (eio_valid(fname))
  ...
  else
    {
      ...
      warn("****************************************");
#if 0
      fatal(" thread %d SimpleScalar/Alpha only supports binary execution on\n"
	    "       little-endian hosts, use EIO files on big-endian hosts",threadid);
#endif
    }
...

  {
    ...
    current->ld_target_big_endian = abfd->xvec->byteorder_big_p;

    debug(" thread %d processing %d sections in `%s'...",threadid,
	...
	debug(" thread %d processing section `%s', %d bytes @ 0x%08x...",threadid,
	...
	if ((bfd_get_section_flags(abfd, sect) & SEC_ALLOC)
	    ...
	    && bfd_section_size(abfd, sect))
	  {
	    ...
	    if (!p)
	      fatal(" thread %d cannot allocate %d bytes for section `%s'",threadid,
		    bfd_section_size(abfd, sect),
		    bfd_section_name(abfd, sect));

	    if (!bfd_get_section_contents(abfd, sect, p, (file_ptr)0,
					  bfd_section_size(abfd, sect)))
	      fatal(" thread %d could not read entire `%s' section from executable",threadid,
		    bfd_section_name(abfd, sect));
	...
	/* zero out the section if it is loadable but not allocated in exec */
	...
	else
	  {
	    ...
	    debug(" thread %d ignoring section `%s' during load...",threadid,
		  bfd_section_name(abfd, sect));
	  }
	...
	/* expected data sections */
	...
	else
	  {
	    ...
	    fatal(" thread %d encountered unknown section `%s', %d bytes @ 0x%08x",threadid,
		  bfd_section_name(abfd, sect), bfd_section_size(abfd, sect),
		  bfd_section_vma(abfd, sect));
	  }
      }
    ...
    current->ld_prog_entry = bfd_get_start_address(abfd);
    current->ld_data_size = data_break - current->ld_data_base;

    /* done with the executable, close it */
    if (!bfd_close(abfd))
      fatal(" thread %d could not close executable `%s'",threadid, argv[0]);
  }
...

  {
    ...
    if (fhdr.f_magic == MD_SWAPH(ECOFF_ALPHAMAGIC))
      ...
      fatal(" thread %d Alpha simulator cannot run PISA binary `%s'",threadid, argv[0]);
    else
      fatal(" thread %d bad magic number in executable `%s' (not an executable)",threadid,
	    argv[0]);

    if (fread(&ahdr, sizeof(struct ecoff_aouthdr), 1, fobj) < 1)
      fatal(" thread %d cannot read AOUT header from executable `%s'",threadid, argv[0]);
    ...
    current->regs->regs_R[MD_REG_GP] = MD_SWAPQ(ahdr.gp_value);
	...
	if (fseek(fobj, floc, 0) == -1)
	  fatal(" thread %d could not reset location in executable",threadid);
	if (fread(&shdr, sizeof(struct ecoff_scnhdr), 1, fobj) < 1)
	  fatal(" thread %d could not read section %d from executable",threadid, i);
	floc = ftell(fobj);
	...
	switch (MD_SWAPW(shdr.s_flags))
	  ...
	  case ECOFF_STYP_TEXT:
	    ...
	    if (fseek(fobj, MD_SWAPQ(shdr.s_scnptr), 0) == -1)
	      fatal(" thread %d could not read `.text' from executable",threadid, i);
	    if (fread(p, MD_SWAPQ(shdr.s_size), 1, fobj) < 1)
	      fatal(" thread %d could not read text section from executable",threadid);
...
#if 0
	    /* create tail padding and copy into simulator target memory */
	    mem_bzero(mem_access, current->mem,
	  ...
	  case ECOFF_STYP_FINI:
		...
		/* copy program section into simulator target memory */
		mem_bcopy(mem_access, current->mem,
		...
		free(p);
	      ...
	      warn(" thread %d section `%s' is empty...",threadid, shdr.s_name);
	    break;
	  ...
	  default:
	    warn(" thread %d section `%s' ignored...",threadid, shdr.s_name);
	  }
      }
  ...
  current->sim_swap_bytes = (endian_host_byte_order() != endian_target_byte_order(j));
  if (current->sim_swap_bytes)
    {
	 ...
	 of this comment, e-mail me for details... */
      fprintf(stderr, " thread %d sim: *WARNING*: swapping bytes to match host...\n",threadid);
      fprintf(stderr, " thread %d sim: *WARNING*: swapping may break your program!\n",threadid);
      /* #else */
      fatal(" thread %d binary endian does not match host endian",threadid);
#endif
    }
  current->sim_swap_words = (endian_host_word_order() != endian_target_word_order(j));
  if (current->sim_swap_words)
    {
#if 0 /* FIXME: disabled until further notice... */
      /* cross-endian is never reliable, why this is so is beyond the scope
	 of this comment, e-mail me for details... */
      fprintf(stderr, " thread %d sim: *WARNING*: swapping words to match host...\n",threadid);
      fprintf(stderr, " thread %d sim: *WARNING*: swapping may break your program!\n",threadid);
      /* #else */
      fatal(" thread %d binary endian does not match host endian",threadid);
#endif
...
#if 0
  sp = ROUND_DOWN(current->ld_stack_base - MD_MAX_ENVIRON, sizeof(MD_DOUBLE_TYPE));
#endif
  ...
  /* write [argc] to stack */
  temp = MD_SWAPQ(argc);
  mem_access(current->mem, Write, sp, &temp, sizeof(qword_t));
  current->regs->regs_R[MD_REG_A0] = temp;
  sp += sizeof(qword_t);

  /* skip past argv array and NULL */
  argv_addr = sp;
  current->regs->regs_R[MD_REG_A1] = argv_addr;
  ...
  for (i=0; i<argc; i++)
    {
      ...
      /* and the data */
      mem_strcpy(mem_access, current->mem, Write, sp, argv[i]);
      sp += strlen(argv[i])+1;
    }
  ...
  for (i = 0; envp[i]; i++)
    {
      ...
      mem_access(current->mem, Write, envp_addr + i*sizeof(md_addr_t),
		 &temp, sizeof(md_addr_t));
      /* and the data */
      mem_strcpy(mem_access, current->mem, Write, sp, envp[i]);
      sp += strlen(envp[i]) + 1;
    }
